...
############# Fig_S7-8_seasonal effect on substrates and microbial community ##################
str(dat)
...
ggplot(dat, aes(factor(year), soil.water_dry*100, fill = treatment)) +
  ...
  scale_fill_manual(limits = c("control", "warmed"), labels = c("Control", "Warmed"), values = c('blue2', 'red2'),name=NULL) +
  scale_y_continuous(limits=c(0,120),n.breaks=4,guide = "prism_offset_minor",expand = c(0, 0))+
...
getwd()
ggsave("Fig_S8_Seasonal character of functional genes_2019-2023_new1.pdf", figure, width =11, height =10,
       device=cairo_pdf)
...
R1517N <- ggplot(dat_N[dat_N$layer %in% c('Oa+e','0-10 cm'),],aes(year,R15_min,fill=treatment))+
  ...
  geom_point(pch=1,position=position_dodge(0.6))+
  facet_grid(layer~.,labeller = labeller(layer=c('Oa+e'='O horizon','0-10 cm'='0-10 cm')))+
...
### 考虑容重
mean(c(13/17-1,38/44-1, 16/24-1, 10/12-1))
mean(c(20/22-1,42/42-1, 27/38-1, 12/14-1))
...
ggplot(dat_M[dat_M$variable %notin% c('TDN'),],
       aes(variable,log(effect_size+1)))+
...
ggplot(dat_M[dat_M$variable %notin% c('TDN'),],
  ...
  facet_grid(layer~.,labeller = labeller(layer = c('O' = "O horizon", '0-10cm' = "0-10 cm")))+
  geom_hline(yintercept = 0,lty=2,linewidth=0.2)+
  geom_vline(xintercept = 6.5,lty=1,linewidth=0.2)+geom_vline(xintercept = 13.5,lty=1,linewidth=0.2)+
  geom_boxplot(outlier.shape = NA,width = 0.32,alpha = 1,fill='white',linewidth = 0.35)+
  geom_point(position = position_jitter(0.04),pch=1,size=1,alpha=0.79,stroke=0.3)+
...
######## Suplementary Fig_7x8_Seasonal responses of substrates and functional genes to warming #########
# 6-8月6次；9-10月5次。
str(dat_M)
...
ggplot(dat_M[dat_M$variable %notin% c('NO','N2O','TDN') & dat_M$seasonp %notin% NA,],
       aes(variable,effect_size))+
  facet_grid(layer+seasonp~.,labeller = labeller(layer = c('O' = "O horizon", '0-10cm' = "0-10 cm")))+
  geom_hline(yintercept = 0,lty=2,linewidth=0.2)+
...
ggsave(file="Fig_S11_seasonal effect of substrates and functional genes.pdf",p2,width=12,height=8,units="in",dpi=300)
...
{ dat=dat0[dat0$labs %in%c('Day1','Day2'),]
  ...
  ggsave(file="D:/工作目录/202409/Manuscript_kai/Talk_20250503/Data and Code/Fig_6+fig_s6_s7_s10_s11/Fig_S11_heatmap_all_t48hr_new.pdf",
...
############ Fig_S12_Correlations of ln RR with key soil parameters ##############
str(myres)
...
p0<-ggplot(df_c[df_c$layer=='O layer',],aes(env,obs,col=r,fill=r,group = layer))+
  ...
  scale_y_discrete(expand=c(0,0),labels=c('NO.lnr'='lnRR NO',
                                          'N2O.lnr'=expression(paste('lnRR N'[2],'O')),#expression(paste(italic(F),''[NO+N2O]))),
                                          position = 'left')) +
...
ggsave(file="D:/工作目录/202409/Manuscript_kai/Talk_20250503/Data and Code/Fig_6+fig_s6_s7_s10_s11/Fig_S12_response variables.pdf",
